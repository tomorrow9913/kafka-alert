...
async def test_process_success(factory_reset, mocker):
    ...
    mock_discord.send.assert_called_once_with(
        "http://webhook", {"content": "test message"}
    )
...
async def test_process_invalid_provider(factory_reset):
    ...
    payload = {"provider": "unknown", "template": "test", "data": {}}
...
async def test_process_template_content(factory_reset, mocker):
    factory = AlertFactory()
    factory.renderer = MagicMock()
    factory.renderer.render_from_string.return_value = {
